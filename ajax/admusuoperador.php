<?php
session_start();
require_once "../modelos/AdmUsuOperador.php";
...
switch ($_GET["op"])
{
	...
	case 'eliminarUsuOper':
		$rspta=$regAdm->eliminarUsuOper($idUsu);
		//Codificar el resultado utilizando json, porque en este caso se recibe un array de registro
		echo json_encode($rspta);
		break;
	...
	case 'listar':
		...
		while($reg=$rspta->fetch_object()){
			$data[]=Array(
				"0"=>'<button class="btn btn-xs btn-primary" onclick="mostrar('.$reg->idPers.')"><i class="fa fa-pencil"></i></button>'.' <button class="btn btn-xs btn-danger" onclick="eliminarUsuOper('.$reg->idPers.')"><i class="fa fa-trash"></i></button>',
				"1"=>$reg->apeNom,
				"2"=>$reg->profesion,
				"3"=>$reg->nroDni,
				"4"=>$reg->email,
	...
	case 'nvoLugarAcceso':
		...
		$idestabUsu=$_POST['idestabUsu'];

		$rspta=$regAdm->nvoLugarAcceso($idServ,$idNivel,$iddisaUsu,$idredUsu,$idmredUsu,$idestabUsu,$idUsu);
	...
	case 'edtLugarAcceso':
		...
		$rspta=$regAdm->edtLugarAcceso($idDet,$idNivel,$idServ,$idestabUsu);
		echo $rspta ? "Los datos fueron actualizados." : "Los datos No se pudieron actualizar."; 
		break;
