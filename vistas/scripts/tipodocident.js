...
function listar()
{
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax": {
			url: '../ajax/tipodocident.php?op=listar',
			type: "get",
			dataType : "json",
			error: function (e){
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url	: "../ajax/tipodocident.php?op=guardaryeditar",
		type : "POST",
		data : formData,
		contentType : false,
		processData : false,
		success: function(datos)
		{
			...
			{
				case '0':
				  mensaje='Tipo de Documento no se pudo Registrar.';
				  break;
				case '1':
				  mensaje='Tipo de Documento Registrado.';
				  break;
				case 'E':
				  mensaje='El Documento que está Ingresando YA Existe.';
				  break;
			}
...
{
	$.post("../ajax/tipodocident.php?op=mostrar",{idwsdat01 : idwsdat01}, function(data, status)
	{
...
function eliminar(idwsdat01)
{
	bootbox.confirm("¿Está seguro que desea eliminar El Tipo de Documento?", function(result){
		if (result)
		{
			$.post("../ajax/tipodocident.php?op=eliminar", {idwsdat01 : idwsdat01}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
