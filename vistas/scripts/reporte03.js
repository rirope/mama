...
function init()
{
	...
	$.post("../ajax/reporte03.php?op=listaDisas",function(r){
	  $("#iddisa").html(r);
	  //Cargamos los items de redes
	  $.post("../ajax/reporte03.php?op=listaRedes", {iddisa:iddisa}, function(r){
      $("#idred").html(r);
      //Cargamos los items de las microredes
			$.post("../ajax/reporte03.php?op=listaMicRedes", {idred : idred}, function(r){
	      $("#idmred").html(r);
	      //Cargamos los establecimientos
				$.post("../ajax/reporte03.php?op=listaEstablec", {idmred : idmred}, function(r){
			      $("#idestab").html(r);
			      $('#iddisa').val(iddisa);
			      $('#idred').val(idred);
			      $('#idmred').val(idmred);
...
function listar()
{
  ...
  if (idestab=='0'){
	...
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax": {
			url: '../ajax/reporte03.php?op=listar',
